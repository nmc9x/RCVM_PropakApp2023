...
namespace ML.DeviceTransfer.PVCFCRFID.APISAASModel
{
    public class APIController
    {
                ...
                try
                {
                    ...
                    using (HttpWebResponse response = request.GetResponse() as HttpWebResponse)
                    {
                        ...
                        if (jsonResponse != null)
                        {
                            ...
                            {
                                jsonResponse.data.RemoveAll(d => d.IsGranary != "0");
                            }
                        }
                        return jsonResponse;
                    }
            ...
            try
            {
                ...
                try
                {
                    ...
                    using (HttpWebResponse response = request.GetResponse() as HttpWebResponse)
                    {
                        ...
                        //ShareFunction.SaveHistory(InternalLanguage.USBKey + ": " + usbKey, InternalLanguage.Login, "", UserController.LogedInUsername, 
                        //    LoggingType.Info, false);
                        //Linh.Tran_230918: Only get product IsShow = 1
                        if (jsonResponse != null)
                        {
                            if (jsonResponse.data != null && jsonResponse.data.Count > 0)
                            {
                                jsonResponse.data.RemoveAll(d => d.IsShow == 0);
                            }
                        }
                        return jsonResponse;
                    }
        ...
        /// <summary>
        /// Linh.Tran_230811
        /// API: Get Delivery code
        /// Method: GET
        /// </summary>
        /// <param name="userName"></param>
        /// <param name="password"></param>
        /// <returns></returns>
        public static APIGetAgentProductByIdModel.Root APIGetAgentProductById(string _id)
        {
            try
            {
                string serverURL = LinkAPI;
                string pathUrl = "AgentManager/GetAgentProductById";
                string requestUrl = String.Format("{0}/{1}?_idagentproduct={2}",
                                                  serverURL, //0
                                                  pathUrl,//1
                                                  _id//2
                                                  );
                ...
                try
                {
                    ...
                    using (HttpWebResponse response = request.GetResponse() as HttpWebResponse)
                    {
                        if (response.StatusCode != HttpStatusCode.OK)
                            ...
                            response.StatusCode,
                            response.StatusDescription));
                        DataContractJsonSerializer jsonSerializer = new DataContractJsonSerializer(typeof(APIGetAgentProductByIdModel.Root));
                        object objResponse = jsonSerializer.ReadObject(response.GetResponseStream());
                        APIGetAgentProductByIdModel.Root jsonResponse
                        = objResponse as APIGetAgentProductByIdModel.Root;
                        //ShareFunction.SaveHistory(InternalLanguage.USBKey + ": " + usbKey, InternalLanguage.Login, "", UserController.LogedInUsername, 
                        //    LoggingType.Info, false);
                        return jsonResponse;
                    }
